...
function Categories({formData, isHistoricDate}) {
  const { t } = useTranslation();
  const { setError } = useLoading();
  const { selectedRestaurant, selectedRestaurantId, setSampleRestaurantModal } = useUserData();
  const { showCategoriesList, setShowCategoriesList, setShowProvidersList, selectedInventoryCategories, updateInventorySelectedCategories } = useFilterData();
  ...
  const [isCategoryView, setIsCategoryView] = useState(showCategoriesList);
  ...
  useEffect(() => {
    ...
    setIsCategoryView(showCategoriesList);
  }, [showCategoriesList]);

  useEffect(() => {
    if ((selectedInventoryCategories && selectedCategory) || formData?.ingredients?.length > 0) {
      ...
      getIngredientsStock(selectedCategory, 1, isPast);
  ...
  const calculateStock = (ingredient, isHistOnlyParam) => {
    ...
    if (remainingStock > 0) {
      if (haveConditioning1) {
        ...
        remainingStock %= recipeUnitQuantity;
      } 
  ...
  const getIngredientsStock = async (category, page = 1, isHistOnlyParam = isHistOnly) => {
  ...
  const handlePageClick = async (ev) => {
    try {
      await getIngredientsStock(selectedCategory, ev.selected + 1, isHistOnly);
              ...
              stock: {
                ...
                unit_stock: Math.max(
                  ...
                  ((item.stock?.unit_stock ?? 0) +
                    ...
                    //* (ingredient.providers?.find(p => p.is_default)?.conditioning_quantity_1 || 1)
                  )
                ),
              },
              stock_snapshots: {
  ...
  const freezeAllStocks = async () => {
    try {
      ...
      if (result.status === 200) {
        let result = await getIngredientsStock(selectedCategory, currentPage + 1);
    ...
    <>
      ...
      <DeleteModal
        ...
        modalData={{
          title: t(`FreezeStock`),
          description: t(
            `Are you sure you want to freeze stock for this category? You cannot undo this action.`
          ),
      ...
      <Row>
        {isCategoryView ? (
          INGREDIENT_CATEGORIES.map((category, i) => (
            <CategoryCard
              key={i}
              category={category}
              getIngredientsStock={getIngredientsStock}
            />
          ))
        ) : (
          <>
