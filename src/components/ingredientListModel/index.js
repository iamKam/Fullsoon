...
function CustomModal(props) {
  ...
  const getMeals = async () => {
    ...
    try {
      ...
      setIngredients((prev) => [...prev, ...result.ingredients]);
      setLoading(false);
    } catch (error) {
      console.log(error);
    }
  ...
  const onSubmit = async () => {
    ...
    if (fd?.id) {
      result = await request.patch(`/ingredient-lists/${fd?.id}`, payload);
      isUpdate = true;
    } else {
      result = await request.post("/ingredient-lists", payload);
    }
  ...
  const createSelectChange = (obj, { action }) => {
    ...
    if (action === "select-option") {
      setSelectedIngredients(() => obj.details);
    }
  ...
  const loadOptions = async (search) => {
    if (!search || search.length < 2 || search === "") {
      ...
      return {
        options: result?.ingredient_lists?.map((i) => ({
          ...i,
          value: i.name,
          label: i.name,
        })),
    ...
    try {
      ...
      return {
        options: response?.ingredient_lists?.map((i) => ({
          ...i,
          value: i.name,
          label: i.name,
        })),
  ...
  const onDelete = async () => {
    try {
      await request.delete(`/ingredient-lists/${fd?.id}`);
      setIsShow(false);
      props.onHide(false);
      setFd({});
      setSelectedIngredients([]);
            ...
            <Form.Group>
              <div className="mb-3 position-relative">
                ...
                <CreatableAsyncPaginate
                  ...
                  placeholder={t("IngredientListName")}
                  isClearable
                />
              ...
              {!loading &&
                <div>
                  <div className="position-relative ps-4 pe-4">
                    <InfiniteScroll
                      dataLength={ingredients.length - 5}
                      next={getMeals}
                      hasMore={true}
                      height={295}
                      className="row"
                        ...
                        >
                          <label className="checkbox checkbox-meals-list">
                            <input
                              type="checkbox"
                              name={i?.name}
                              checked={selectedIngredients.find((m) => m === i.id)}
                              onChange={selectMeal}
                              value={i?.id}
                            />
         ...
         <Modal.Body>
           <div className="d-flex justify-content-center flex-column  text-center upload-container">
             ...
             <div className="heading mt-5">
               <label>
                 {updated ? t("ingredientUpdated") : t("ingredientCreated")} !
               </label>
             </div>
             <div className="subheading mt-2 mb-5">
               <label>
                 {updated
                   ? t("successfullyIngredientUpdated")
                   : t("successfullyIngredientCreated")}
                 .
               </label>
             </div>
             <div>
